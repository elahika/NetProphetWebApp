...
lars.multi <- function(tdata,rdata,pert,prior,allowed)
{
  cat("hellooo in lars4")
	...
	for (i in targets)
	{
		...
		results.lars <- lars(t(x),tdata[i,mindices],trace=FALSE,max.steps=600,type="lasso",normalize=FALSE,intercept=TRUE);
...
lars.local <- function(tdata,rdata,pert,prior,allowed)
{
  cat("hellooo in lars 8")
  cat(as.character(Sys.time()),"\n")
  B.adj <- matrix(0,nrow=dim(rdata)[1],ncol=dim(tdata)[1])
	for(i in 1:dim(tdata)[1]) {
		...
		x <- rdata[,mindices] * prior[,i]
		x[which(allowed[,i]==0),]<-0;
		lars.paths.cv <-  cv.lars(t(x),tdata[i,mindices],K=10,trace=FALSE,max.steps=600,type="lasso",normalize=FALSE,intercept=TRUE,se=FALSE, plot.it=FALSE);
		minFrac <- lars.paths.cv$index[which.min(lars.paths.cv$cv)]
		lars.paths <- lars(t(x),tdata[i,mindices],trace=FALSE,max.steps=600,type="lasso",normalize=FALSE,intercept=TRUE);
		B.adj[,i] <- predict(lars.paths,s=minFrac,type="coefficients",mode="fraction")$coefficients
		#Scale B.adj according to prior		
